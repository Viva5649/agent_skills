...
ID_CACHE_PATH = str(runtime_file("creator-signal-digest-x-id-cache.json"))
ID_CACHE_TTL_DAYS = 7
...
# ---------------------------------------------------------------------------
# SSL: use certifi CA bundle on macOS where the system keychain may lack
# intermediate certificates (e.g. Let's Encrypt YE2 / ISRG Root YE chain).
# ---------------------------------------------------------------------------

if certifi is not None:
    _certifi_context = ssl.create_default_context(cafile=certifi.where())
    _https_handler = HTTPSHandler(context=_certifi_context)
    install_opener(build_opener(_https_handler))
...
# ---------------------------------------------------------------------------
# Interactive zsh env fallback (for credentials stored in ~/.zshrc / ~/.zshenv)
# ---------------------------------------------------------------------------

_interactive_zsh_env_cache: Optional[Dict[str, str]] = None


def _load_interactive_zsh_env() -> Dict[str, str]:
    """Read X API credentials from interactive zsh environment.

    This fallback is needed when the script runs under OpenClaw's gateway
    (launched via LaunchAgent + sh wrapper), which does not source ~/.zshrc
    or ~/.zshenv.  The interactive zsh shell will source those files.
    """
    try:
    ...
    for line in result.stdout.splitlines():
        ...
        if key in ("GETX_API_KEY", "TWITTERAPI_IO_KEY", "X_BEARER_TOKEN", "TWITTER_API_BACKEND"):
            env_vars[key] = value
    return env_vars


def _get_env(env_name: str) -> Optional[str]:
    """Get env var from interactive zsh first, then fall back to current process.

    Priority order ensures that credentials configured in ~/.zshrc / ~/.zshenv
    are always preferred over whatever happens to be in the current process
    environment (which may be stripped by LaunchAgent or subprocess isolation).
    os.getenv is only used as a last-resort fallback.
    """
    global _interactive_zsh_env_cache
    if _interactive_zsh_env_cache is None:
        _interactive_zsh_env_cache = _load_interactive_zsh_env()
    value = _interactive_zsh_env_cache.get(env_name)
    if value:
        logging.info("%s loaded from interactive zsh fallback", env_name)
        return value
...
def select_backend(backend_name: str, no_cache: bool = False) -> Optional[TwitterBackend]:
    ...
    if backend_name == "twitterapiio":
        key = _get_env("TWITTERAPI_IO_KEY")
        if not key:
    ...
    if backend_name == "auto":
        getx_key = _get_env("GETX_API_KEY")
        if getx_key:
...
Examples:
    ...
    backend_name = args.backend or _get_env("TWITTER_API_BACKEND") or "auto"
    ...
    if not args.output:
        fd, temp_path = tempfile.mkstemp(prefix="creator-signal-digest-x-", suffix=".json")
        os.close(fd)
