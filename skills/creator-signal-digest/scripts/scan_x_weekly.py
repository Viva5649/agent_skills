#!/usr/bin/env python3
"""Scan recent X posts from target creator accounts without using the X API.
...
def score_text(text: str) -> int:
    ...
    for weak_bad in WEAK_NEGATIVE_HINTS:
        if weak_bad in t and not any(good in t for good in ("why", "means", "opportunity", "watch")):
            score -= 1

    score += len(signal_tags(text))
